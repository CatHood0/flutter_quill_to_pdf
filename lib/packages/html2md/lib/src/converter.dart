...
String convert(
  ...
  String? rootTag,
  String? imageBaseUrl,
  bool removeLeadingWhitespaces = true,
  bool escape = true,
  ...
  final output = _process(Node.root(input, rootTag: rootTag, removeLeadingWhitespaces: removeLeadingWhitespaces), escape);
  return _postProcess(output);
}

String _escape(String input) {
  return input
      .replaceAllMapped(RegExp(r'\\(\S)'), (match) => '\\\\${match[1]}') // Escape backslash escapes!
      .replaceAllMapped(RegExp(r'^(#{1,6} )', multiLine: true), (match) => '\\${match[1]}') // Escape headings
      .replaceAllMapped(RegExp(r'^([-*_] *){3,}$', multiLine: true), (match) {
        return match[0]!.split(match[1]!).join('\\${match[1]}');
      })
      .replaceAllMapped(RegExp(r'^(\W* {0,3})(\d+)\. ', multiLine: true), (match) => '${match[1]}${match[2]}\\. ')
      .replaceAllMapped(RegExp(r'^([^\\\w]*)[*+-] ', multiLine: true), (match) {
        return match[0]!.replaceAllMapped(RegExp(r'([*+-])'), (match) => '\\${match[1]}');
      })
      .replaceAllMapped(RegExp(r'^(\W* {0,3})> '), (match) => '${match[1]}\\> ')
      .replaceAllMapped(RegExp(r'\*+(?![*\s\W]).+?\*+'), (match) => match[0]!.replaceAll(RegExp(r'\*'), '\\*'))
      .replaceAllMapped(RegExp(r'_+(?![_\s\W]).+?_+'), (match) => match[0]!.replaceAll(RegExp(r'_'), '\\_'))
      .replaceAllMapped(RegExp(r'`+(?![`\s\W]).+?`+'), (match) => match[0]!.replaceAll(RegExp(r'`'), '\\`'))
...
String _process(Node inNode, bool needEscape) {
  var result = '';
  for (var node in inNode.childNodes()) {
    ...
    if (node.nodeType == 3) {
      ...
      // Element
      replacement = _replacementForNode(node, needEscape);
    }
...
String _replacementForNode(Node node, bool escape) {
  var rule = Rule.findRule(node);
  if (rule.append != null) {
    _appendRuleSet.add(rule);
  }
  var content = _process(node, escape);
