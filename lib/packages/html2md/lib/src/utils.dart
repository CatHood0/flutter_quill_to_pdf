...
bool hasVoid(dom.Node? node) {
  return node is dom.Element && _asElement(node)!.querySelectorAll(_kVoidElements.join(',')).isNotEmpty;
}
...
dom.Node prepareRoot(dom.Node rootNode, bool removeLeadingWhitespaces) {
  var result = _collapseWhitespace(rootNode, removeTags, removeLeadingWhitespaces);
  return result;
}
...
dom.Node _collapseWhitespace(dom.Node domNode, List<String> removeTags, bool removeLeadingWhitespaces) {
  ...
  while (current != domNode) {
    if (current!.nodeType == 3 || current.nodeType == 4) {
      ...
      var text = removeLeadingWhitespaces ? textNode.data.replaceAll(RegExp(r'[ \r\n\t]+'), ' ') : textNode.data;
      if ((prevText == null || RegExp(r' $').hasMatch(prevText.data)) && text.substring(0, 1) == ' ') {
...
bool _isPre(dom.Node? node) {
  return node is dom.Element && _asElement(node)!.localName!.toLowerCase() == 'pre';
}
