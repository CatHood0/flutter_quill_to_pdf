class Constant {
  ...
  static final RegExp EMPTY_ALIGNED_P =
      RegExp(r'^<p\s+style="text-align:(center|right|left|justify)">(([<br>]+)|(<span.*?>[<br>]+<\/span>))<\/p>$');
  static final RegExp ALIGNED_P_PATTERN = RegExp(r'<p\s?style="text-align:(center|right|justify|left)">(.+?)<\/p>');
  static final RegExp ALIGNED_P_INDENTED_PATTERN =
      RegExp(r'<p\s?style="padding-left:(\d+)em;text-align:(center|right|left|justify)">(.*)<\/p>');
  static final RegExp ALIGNED_P_IMAGE_PATTERN =
      RegExp(r'<img\s*style="max-width:\s*(.*?)%;object-fit:\s*(contain|cover|none|fill)"\s*src="(.+)">');
  static final RegExp ALIGNED_HEADER_PATTERN = RegExp(r'<h([1-6]) style="text-align:(center|right|left|justify)">(.+)<\/h\1>');
  ...
  static final RegExp LINK_ALIGNED_PATTERN = RegExp(r'(.*?)(<a\s+href="(.+?)"\s+target="(.+?)">(.*?)<\/a>)|((.*?)<\/p>)');
  ...
  static final RegExp LINK_PATTERN =
      RegExp(r'\[(.+?)\]\s?\(([(http(s)?):\/\/(www\.)?a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*))\)');
  static final RegExp LIST_PATTERN = RegExp(r'^(\d+|\w+\.|[*+-](?![*]))\s{1,1}(.+)');
  static final RegExp INLINE_STYLES_PATTERN = RegExp(r'(\*\*\*|\*\*|\*|_)(.+?)(\1)');
  static final RegExp INLINE_MATCHER = RegExp(
      r'((\*\*\*(?<boldItalic>(?:(?!\*\*\*).)+)\*\*\*)|(\*\*(?<bold>(?:(?!\*\*).)+)\*\*)|(\*(?<italic>[^*]+)\*)|(\_(?<underline>[^_]+)\_)|(\*\*\*\_(?<boldItalicUnderline>(?:(?!\_\*\*\*).)+)\_\*\*\*))|(\[(?<linkTitle>[^]]+)]\((?<linkHref>.*)\)|(<a\sstyle="((wiki-doc:\s*(.*?);)?)((line-height:\s*(.*?);?)?)((font-family:\s*(.*?);?)?)(font-size:\s*(\d+.\d+|\d+))?"\shref="(.+?)"\s?target="(.+?)">(.+?)<\/a>))|~~(?<strike>(?:(?!~~).)+)~~');

  //Wrong and invalids patterns
  static final RegExp INVALID_HEADER_PATTERN = RegExp(r'^\s{0,}(#){1,}((?!\w|\d+)\s{0,}?(#)+)+$'); // detect some strnigs like: # # # # #
  static final RegExp INVALID_TOGGLEABLE_STYLES_PATTERN =
      RegExp(r'^\s{0,}(\*)+((?!\w|\d+)\s{0,}?(\*)+)+$'); // detect some strnigs like: * * * * *
